...
import { EmittedFile, ExtraEntryPoint } from '../models';
import { interpolateEnvironmentVariablesToIndex } from '../nx/interpolate-env-variables-to-index';
import { generateEntryPoints } from '../nx/package-chunk-sort';
...
export class WriteIndexHtmlPlugin {
    ...
    private getEmittedFiles(compilation: webpack.Compilation): EmittedFile[] {
        ...
        // dedupe
        return files.filter(
            ({ file, name }, index) => files.findIndex((f) => f.file === file && (!name || name === f.name)) === index
        );
    }
    ...
    private augmentIndexHtml(params: {
        ...
        for (const script of scripts) {
            ...
            // such as runtime.js
            const scriptPredictor = ({ file }: { file: string; name: string; extension: string }): boolean =>
                file === script;
        ...
        if (typeof params.baseHref == 'string') {
            ...
            if (!baseElement) {
                baseElement = treeAdapter.createElement('base', undefined, [{ name: 'href', value: params.baseHref }]);
    ...
    private filterAndMapBuildFiles(
        ...
        for (const { file, name, extension, initial } of files) {
            if (name && initial && extensionFilter.includes(extension) && !hotUpdateAsset.test(file)) {
