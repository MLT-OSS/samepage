...
import { Configuration, ids, RuleSetRule, WebpackPluginInstance } from 'webpack';
...
import { ExtraEntryPointClass, NormalizedWebpackExecutorOptions } from '@nx/webpack/src/executors/webpack/schema';
 ...
 */
export function xmWithWeb(pluginOptions: WithWebOptions = {}): NxWebpackPlugin {
    return function configure(
        ...
        if (!pluginOptions.ssr) {
            plugins.push(new webpack.DefinePlugin(getClientEnvironment(process.env.NODE_ENV).stringified));
        }
        ...
        const hashFormat = getOutputHashFormat(mergedOptions.outputHashing as string);

        const includePaths: string[] = [];
        if (mergedOptions?.stylePreprocessorOptions?.includePaths?.length > 0) {
            mergedOptions.stylePreprocessorOptions.includePaths.forEach((includePath: string) =>
        ...
        if (mergedOptions?.styles?.length && mergedOptions.styles.length > 0) {
            normalizeExtraEntryPoints(mergedOptions.styles!, 'styles').forEach((style) => {
        ...
        config.output = {
            ...config.output,
            crossOriginLoading: mergedOptions.subresourceIntegrity ? ('anonymous' as const) : (false as const)
        };

        // In case users customize their webpack config with unsupported entry.
        if (typeof config.entry === 'function') throw new Error('Entry function is not supported. Use an object.');
        if (typeof config.entry === 'string') throw new Error('Entry string is not supported. Use an object.');
        if (Array.isArray(config.entry)) throw new Error('Entry array is not supported. Use an object.');
        ...
        (config.plugins as any[]).push(...plugins);

        (config.resolve as any).mainFields = ['browser', 'module', 'main'];
...
function createScriptsPlugin(options: MergedOptions): WebpackPluginInstance[] {
    ...
    const globalScriptsByBundleName = normalizeExtraEntryPoints(options.scripts || [], 'scripts').reduce(
        (prev: { inject: boolean; bundleName: string; paths: string[] }[], curr) => {
...
function getCommonLoadersForCssModules(options: MergedOptions, includePaths: string[]) {
    // load component css as raw strings
    return [
        {
            loader: options.extractCss ? MiniCssExtractPlugin.loader : require.resolve('style-loader')
        },
        {
...
function getCommonLoadersForGlobalCss(options: MergedOptions, includePaths: string[]) {
    return [
        {
            loader: options.extractCss ? MiniCssExtractPlugin.loader : require.resolve('style-loader')
        },
...
function getCommonLoadersForGlobalStyle(options: MergedOptions, includePaths: string[]) {
    return [
        {
...
function postcssOptionsCreator(
    ...
    { includePaths, forCssModules = false }: { includePaths: string[]; forCssModules?: boolean }
) {
    const hashFormat = getOutputHashFormat(options.outputHashing as string);
