...
import { createCopyPlugin } from '../plugins/create-copy-plugin';
...
const IGNORED_WEBPACK_WARNINGS = [/The comment file/i, /could not find any license/i];
 ...
 */
export function xmWithNx(pluginOptions?: WithNxOptions): NxWebpackPlugin {
    return function configure(
        ...
        if (options.main) {
            const mainEntry = options.outputFileName ? path.parse(options.outputFileName).name : 'main';
        ...
        if (options.polyfills) {
            entry['polyfills'] = [...(entry['polyfills'] || []), path.resolve(options.root!, options.polyfills)];
        }
        ...
        const filename = options.outputHashing ? `[name]${hashFormat.script}.js` : '[name].js';
        const chunkFilename = options.outputHashing ? `[name]${hashFormat.chunk}.js` : '[name].js';

        const updated = {
            ...config,
            context: context ? path.join(context.root, options.projectRoot!) : undefined,
            ...
            cache: options.target === ('node' as const) && options.watch ? { type: 'memory' as const } : undefined,
            devtool:
            ...
            optimization: {
                ...
                sideEffects: true,
                minimize:
                    typeof options.optimization === 'object' ? !!options.optimization.scripts : !!options.optimization,
                minimizer: [
                    options.compiler !== 'swc'
                        ? new TerserPlugin({
                              ...
                              terserOptions: {
                                  ...
                                  ecma: getTerserEcmaVersion(join(options.root!, options.projectRoot!)),
                                  safari10: true,
                                  format: {
                                      ascii_only: true,
                                      comments: false,
                                  ...
                                  compress: {
                                      drop_console: process.env.NX_TASK_TARGET_CONFIGURATION === 'production'
                                  }
                              },
...
export function createLoaderFromCompiler(options: NormalizedWebpackExecutorOptions) {
    switch (options.compiler) {
        ...
        case 'babel':
            ...
            const babelConfig = {
                test: /\.([jt])sx?$/,
                loader: path.join(__dirname, './web-babel-loader.ts'),
                exclude: /node_modules/,
                options: {
