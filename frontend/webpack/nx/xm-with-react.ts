...
function addHotReload(config: Configuration) {
    if (config.mode === 'development' && config['devServer']?.hot) {
        ...
        const babelLoader = config?.module?.rules?.find(
            (rule) => rule && typeof rule !== 'string' && rule.loader?.toString().includes('babel-loader')
        );

        if (babelLoader && typeof babelLoader !== 'string') {
            (babelLoader.options as any)['plugins'] = [
        ...
        (config.plugins as any[]).push(new ReactRefreshPlugin({ overlay: false }));
    }
}
...
function removeSvgLoaderIfPresent(config: Configuration) {
    ...
    config?.module?.rules?.splice(svgLoaderIdx as number, 1);
}
 ...
 */
export function xmWithReact(pluginOptions: WithReactOptions = {}) {
    return function configure(config: Configuration, context: NxWebpackExecutionContext): Configuration {
        const { xmWithWeb: withWeb } = require('./xm-with-web');
