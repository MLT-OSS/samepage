...
public class SmsServiceImpl implements SmsService {
    ...
    public void sendEmail(String mail,
                          ...
                          String title,
                          String code) {
        send(mail, subject, title, code);
    }

    @Override
    public void sendEmail(String temp, String subject,
        ...
        String process = templateEngine.process(temp, context);
        try {
            ...
            throw new BusinessException(RespCode.INTERNAL_SERVER_ERROR, "发送邮箱验证码失败");
        }

    }

    private String getCaptchaTemp(String code, String title) {
        Context context = new Context();
        //设置模板所需的参数
    ...
    @Value("${mail.code.from:#{null}}")
    private String from;


    private void send(String mail, String subject, String title, String code) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true);
            log.info("from:{}",from);
            ...
            helper.setSubject(subject);
            //设置发送邮件的内容 第二个设置为true则可以发送带HTML的邮件
            helper.setText(getCaptchaTemp(code, title),
                    true);
            mailSender.send(message);
        } catch (MessagingException e) {
            log.error(e.getMessage(), e);
