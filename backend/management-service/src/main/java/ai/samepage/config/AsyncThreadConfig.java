...
public class AsyncThreadConfig {
    ...
    @Value("${sys.async-pool.corePoolSize:4}")
    private int corePoolSize;

    @Value("${sys.async-pool.maxPoolSize:4}")
    private int maxPoolSize;

    @Value("${sys.async-pool.keepAliveSeconds:100}")
    private int keepAliveSeconds;

    @Value("${sys.async-pool.queueCapacity:150}")
    ...
    public ExecutorService asyncThreadExecutor() {
        ...
        //线程名前缀
        threadPool.setThreadNamePrefix("--asyncThreadPool--");
        ...
        threadPool.setRejectedExecutionHandler((r, executor) -> {
            ...
            thread.start();
            log.warn("asyncThreadPool 线程池已经达到极限, 丢弃任务已交给主线程执行 {} ", thread.getName());
        });
