...
import ai.samepage.common.core.exception.StateCodeException;
import ai.samepage.common.core.resp.ErrorBody;
...
public class GlobalFacadeAdvice {
    ...
    public RespBody<ErrorBody> nullPointerException(NullPointerException e) {
        ...
        return RespBody.error(RespCode.INTERNAL_SERVER_ERROR, errorBody);
    }
    ...
    public RespBody<ErrorBody> httpStatusCodeException(HttpStatusCodeException e) {
        ...
        errorBody.setMetadata(metadata);
        return RespBody.error(statusCode.value(), statusCode.getReasonPhrase(), errorBody);
    }
    ...
    public RespBody<ErrorBody> validateException(ValidateException e) {
        log.error("错误信息：", e);
        return RespBody.error(BAD_REQUEST, e.getMessage(), null);
    }
    ...
    public RespBody<ErrorBody> httpMessageNotReadableException(HttpMessageNotReadableException e) {
        log.error(StrUtil.format("http 消息不可读异常信息：{}", e.getMessage()), e);
        ErrorBody errorBody = ErrorBody.build(e);
        if (e.getCause() instanceof InvalidFormatException) {
            ...
            log.error("requiredType:{},targetValue:{}", fe.getTargetType().getName(), fe.getValue());
        }
        return RespBody.error(BAD_REQUEST, errorBody);
    }
     ...
     * 服务器数据库发生异常
     */
    @ExceptionHandler(value = {BadSqlGrammarException.class, SQLTransientConnectionException.class})
