 ...
 */
package ai.samepage.common.file;
...
public class IS3ServiceImpl implements IS3Service {
    ...
    public IS3ServiceImpl(IS3Properties properties,
                          ThreadPoolExecutor executor,
                          Integer spitSize) {
        if (Objects.isNull(properties)
                || StrUtil.isBlank(properties.getAccessKey())
                || StrUtil.isBlank(properties.getSecretKey())
                || StrUtil.isBlank(properties.getRegion())
                || StrUtil.isBlank(properties.getEndpoint())
        ) {
            log.error("构建对象存储客户端参数不正确");
            throw new IllegalArgumentException("构建对象存储客户端参数不正确");
        }
    ...
    private void s3Upload(long tid, String objectId, Map<String, Object> headers, File file) {
        ...
        try {
            PutObjectRequest putObjectRequest = new PutObjectRequest(bucketName, objectId, file);
    ...
    private void s3Upload(long tid, String objectId, long fileSize, Map<String, Object> headers, InputStream inputStream) {
        ...
        long uploadStart = System.currentTimeMillis();
        try {
            PutObjectRequest putObjectRequest = new PutObjectRequest(bucketName, objectId, inputStream, objectMetadata);
    ...
    private static Integer MAX_EXPIRE_SECOND = 7 * 24 * 60 * 60;

    @Override
    public URL downloadPresignedUrl(String objId, Integer expireSecond, Integer cacheMinute)
            throws IllegalArgumentException {
        return downloadPresignedUrl(objId, expireSecond, null, null, cacheMinute);
    }


    @Override
    public URL downloadPresignedUrl(String objId, Integer expireSecond, String contentType, String contentDisposition, Integer cacheMinute) throws IllegalArgumentException {
        return this.downloadPresignedUrl(objId, expireSecond, contentType, contentDisposition, cacheMinute, true);
    }

    @Override
    public URL downloadPresignedUrl(String objId,
                                    Integer expireSecond,
                                    String contentType,
                                    String contentDisposition,
                                    Integer cacheMinute, boolean isCdn) throws IllegalArgumentException {
        if (StrUtil.isBlank(objId) || expireSecond <= 0 || expireSecond > MAX_EXPIRE_SECOND) {
            throw new IllegalArgumentException("获取预签名的下载的url参数不正确");
        }
        GeneratePresignedUrlRequest request = new GeneratePresignedUrlRequest(bucketName, objId);
    ...
    public String uploadPresidedUrl(String objId, String contentType, Long expireSecond, String version)
        ...
        GeneratePresignedUrlRequest request = new GeneratePresignedUrlRequest(bucketName, objId, HttpMethod.PUT)
    ...
    public InputStream download(String objectId) throws BusinessException {
        ...
        try {
            ...
            return s3.getObject(bucketName, objectId).getObjectContent();
        } catch (Exception e) {
    ...
    private static final int BUFFER_SIZE = 1024 * 2 * 1024;


    private void downloadSplitFile(String objKey, long start, long end, RandomAccessFile accessFile) throws Exception {
        if (start >= end) {
            return;
        }
        S3ObjectInputStream objectContent = s3.getObject(new GetObjectRequest(bucketName, objKey)
                .withRange(start, end)).getObjectContent();
        try (InputStream inputStream = objectContent.getDelegateStream()) {
    ...
    public boolean existsObj(String objId) {
        if (StrUtil.isNotBlank(objId)) {
            return s3.doesObjectExist(bucketName, objId);
        }
        return false;
    }


    private Date expire(Integer expireSeconds) {
    ...
    public void deleteFile(String objectId) {
        s3.deleteObject(bucketName, objectId);
    }
